...
pygame.draw.polygon(tough_invader_surface, DARK_GREEN, [(0, 15), (5, 10), (5, 20)]) # Left arm
pygame.draw.polygon(tough_invader_surface, DARK_GREEN, [(40, 15), (35, 10), (35, 20)]) # Right arm
...
powerup_imgs["fever"] = fever_surface # Corrected variable name
...
class Invader:
    def __init__(self, invader_type, x, y):
        ...
        # REDUCED FIRE RATE for testing (more frequent shots)
        self.fire_rate = random.randint(60, 180) # Frames between shots (1 to 3 seconds)
        self.last_shot_time = pygame.time.get_ticks()

        if self.type == "tough":
            self.health = 3
            self.x_change = 1 # Slower
            self.fire_rate = random.randint(100, 250) # Tougher, slightly less frequent but potentially more dangerous shots
        elif self.type == "fast":
            self.x_change = 4 # Faster
            self.fire_rate = random.randint(40, 120) # Faster, more frequent shots
        elif self.type == "boss":
            self.health = 10
            self.x_change = 0.5 # Very slow, but big
            self.y_change = 10 # Moves less vertically
            self.fire_rate = random.randint(30, 90) # Boss fires very frequently
...
class PowerUpItem:
    def __init__(self, power_up_type, x, y):
        ...
        self.image = powerup_imgs[power_up_type] # Corrected to use powerup_imgs
        self.x = x
        self.y = y
...
while running:
    ...
    for invader_obj in list(invaders): # Iterate over a copy to allow removal
        ...
        # Removed the Y-position constraint for more immediate shooting
        # if invader_obj.y < SCREEN_HEIGHT - 150: # Original condition
